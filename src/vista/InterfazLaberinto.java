...
public class InterfazLaberinto {
    ...
    private void intentarMover(int desplazamientoX, int desplazamientoY) {
        ...
        if (mostrarPreguntaConTiempo()) {
            ...
            // Efectos de casilla
            if (matriz[destinoY][destinoX] == 1) { // Botiquín
                jugador.curar(vidaBotiquin);
            ...
            // Si la vida es 0 o menos, fin del juego
    ...
    private boolean mostrarPregunta() {
        ...
        String respuestaCorrecta = "4";
        JTextField respuesta = new JTextField();
        Object[] mensaje = {pregunta, "Respuesta:", respuesta};
        int option;
        do {
            option = JOptionPane.showConfirmDialog(null, mensaje, "Pregunta", JOptionPane.OK_CANCEL_OPTION);
            if (option == JOptionPane.OK_OPTION && respuesta.getText().trim().equalsIgnoreCase(respuestaCorrecta)) {
                return true;
            }
    ...
    private boolean mostrarPreguntaConTiempo() {		//!!!!!!!!!!!!!!! METER MAS PREGUNTAS
        // Ejemplo simple 
        String pregunta = "¿Cuánto es 2 + 2?";
        String respuestaCorrecta = "4"; 
        JTextField respuesta = new JTextField();
        Object[] mensaje = {pregunta, "Tiempo: " + tiempoPregunta + " segundos", "Respuesta:", respuesta};
        ...
        Timer timer = new Timer(1000, null); // Crea un timer que "marca" cada segundo 
        ...
        respuesta.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent e) {
                if (respuesta.getText().trim().equalsIgnoreCase(respuestaCorrecta)) {
                    resultado[0] = true;
                    timer.stop();
                    dialog.dispose();
                }
        ...
        // Si no acertó, daño por pregunta
        if (!resultado[0]) {
            jugador.reducirVida(danoPregunta); 
            JOptionPane.showMessageDialog(frame, "¡Respuesta incorrecta o tiempo agotado! Pierdes " + danoPregunta + " de vida.");
            actualizarVista();
        }
