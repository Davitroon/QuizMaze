...
public class CreateMazeUI {
	...
	private void initialize() {
		...
		SpinnerNumberModel questionTimeModel = new SpinnerNumberModel(minQuestionTime, minQuestionTime, maxQuestionTime, 5);
		...
		btnUpdate.addActionListener(e -> {
		    ...
		    for (int i = 0; i < cols; i++) tableColumns[i] = "Col " + (i + 1);
		    ...
		    mazeTable.addMouseListener(new java.awt.event.MouseAdapter() {
		        public void mouseClicked(java.awt.event.MouseEvent evt) {
		            ...
		            if ((row == 0 && col == 0) || (row == lastRow && col == lastCol)) return;

		            Object value = mazeTable.getValueAt(row, col);
		            if (value == null || value.toString().trim().isEmpty()) mazeTable.setValueAt("3", row, col);
		            else if (value.toString().trim().equals("3")) mazeTable.setValueAt("", row, col);
		        }
		    });
		});
	    ...
	    btnCreate.addActionListener(new ActionListener() {
	    	public void actionPerformed(ActionEvent e) {
	    		...
	    		for (int i = 0; i < rows; i++)
	    			for (int j = 0; j < cols; j++)
	    				if ("3".equals(mazeTable.getValueAt(i, j))) map[i][j] = 3;

	    		maze = new Maze(width, height, numCrocodiles, crocodileDamage, numMedkits,
	    				medkitHeal, questionTime, questionDamage, numQuestions, map);
	    		disposition = new Disposition(maze.getMapa(), maze.getId(), model);
	    		...
	    		disposition.setIdLaberinto(maze.getId());
